...
setup_agora_params.py that it calls to generate params for the tests.
...
The steps/results for this test are:
    ...
    1.  Set-up simulation directories for several tests/several ChaNGa 
        installations (setup_agora_params.py)
    ...
    2.  Compile ChaNGa for the different tests.  The tests in general can 
        ...
        directories.
        i.  I also have my own configure scripts that I use to override the
            default ChaNGa scripts. (see setup_agora_params.py)  This will
            temporarily replace the ChaNGa scripts (only during the building
            phase).  configure scripts are housed in sub-directories under
            userconfig/
    3.  Run the tests with ChaNGa
